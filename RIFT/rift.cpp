...
int main (int argc, char *argv[])
{
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr scene_cloudIntensity(new pcl::PointCloud<pcl::PointXYZI>);
  ...
  // Object for storing the point cloud with intensity value.
  pcl::PointCloud<pcl::PointXYZI>::Ptr model_cloudIntensity(new pcl::PointCloud<pcl::PointXYZI>);
  // Object for storing the intensity gradients.
...
// Convert the RGB to intensity.
  pcl::PointCloudXYZRGBtoXYZI(*model, *model_cloudIntensity);
  pcl::PointCloudXYZRGBtoXYZI(*scene, *scene_cloudIntensity);
  ...
  pcl::NormalEstimation<pcl::PointXYZI, NormalType> norm_est;
  norm_est.setInputCloud (model_cloudIntensity);
  norm_est.setRadiusSearch(0.03);
  pcl::search::KdTree<pcl::PointXYZI>::Ptr kdtree(new pcl::search::KdTree<pcl::PointXYZI>);
  norm_est.setSearchMethod(kdtree);

  norm_est.compute (*model_normals);
 //std::cout << "Inicio del Calculo de las Normales2\n" << std::endl;
  pcl::NormalEstimation<pcl::PointXYZI, NormalType> norm_est2; //-----------------------------------------
  norm_est2.setInputCloud (scene_cloudIntensity);
  ...
  std::cout << "Inicio del Calculo de los Gradientes\n" << std::endl;
  ...
  ge.setInputCloud(model_cloudIntensity);
  ge.setInputNormals(model_normals);
  ge.setRadiusSearch(0.03);
  ge.compute(*model_gradients);

// Compute the intensity gradients.
  pcl::IntensityGradientEstimation < pcl::PointXYZI, pcl::Normal, pcl::IntensityGradient, pcl::common::IntensityFieldAccessor<pcl::PointXYZI> > ge2;
  ge2.setInputCloud(scene_cloudIntensity);
  ge2.setInputNormals(scene_normals);
  ge2.setRadiusSearch(0.03);
  ge2.compute(*scene_gradients);


  std::cout << "Fin del Calculo de los Gradientes\n" << std::endl;
  ...
  // std::cout << "Fin del muestro de los Keypoints\n" << std::endl;
 ...
 pcl::RIFTEstimation<pcl::PointXYZI, pcl::IntensityGradient, DescriptorType>  descr_est2;
   descr_est.setInputCloud(scene_cloudIntensity);
  descr_est.setSearchMethod(kdtree);
  // Set the intensity gradients to use.
  descr_est.setInputGradient(scene_gradients);
  // Radius, to get all neighbors within.
  descr_est.setRadiusSearch(descr_rad_);
  // Set the number of bins to use in the distance dimension.
  descr_est.setNrDistanceBins(4);
